...
// Personaje agrupa los datos de cualquier combatiente
...
// Habilidad: nombre y cuánto bonus de ataque da
...
var habilidadesDragon = []Habilidad{
	{"Zarpazo", 5},        // Débil pero común
	{"Llamarada", 15},     // Daño medio
	{"Rugido Mortal", 25}, // La más peligrosa
}
...
func (p *Personaje) Atacar(objetivo *Personaje) {
	...
	// Variación aleatoria ±5: rand.Intn(11) da 0-10, restamos 5 = rango -5 a +5
...
// ===== PARTE 4: Función para elegir habilidad =====

func elegirHabilidad(habilidades []Habilidad) Habilidad {
	fmt.Println("Elige tu habilidad:")

	// range nos da índice (i) y valor (h) de cada elemento del slice
	...
	// Si el jugador escribe un número fuera de rango, usamos la primera habilidad
	if opcion < 1 || opcion > len(habilidades) {
		fmt.Println("Opción inválida, se usó:", habilidades[0].Nombre)
		return habilidades[0]
	}

	return habilidades[opcion-1] // El slice es 0-based, el menú es 1-based
}

// ===== PARTE 5: main (el motor del juego) =====
